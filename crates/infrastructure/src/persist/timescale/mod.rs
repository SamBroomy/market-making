...
use domain::{
    ...
    services::persistence::{DataWriterConfig, MarketDataWriter},
};
...
use crate::persist::timescale::config::TimescaleSettings;
...
#[async_trait]
impl MarketDataWriter for TimescaleWriter {
    async fn from_config(config: &dyn DataWriterConfig) -> Result<Self>
    where
        Self: Sized,
    {
        let pool = PgPool::connect(config.connection_string().expose_secret()).await?;
        sqlx::migrate!("./../../migrations")
            .run(&pool)
            .await
            .expect("Failed to run database migrations");
        info!("Connected to TimescaleDB at {:?}", config);
    ...
    async fn write_depth_snapshot(
        ...
        .await
        {
            error!(
                symbol = %symbol,
                error = %e,
                "Failed to insert depth snapshot"
            );
            return Err(e.into());
        }
        Ok(())
    ...
    async fn write_window_ticker(&self, ticker: &WindowTickerData) -> Result<()> {
        ...
        .await
        {
            error!(
                symbol = %ticker.symbol,
                error = %e,
                "Failed to insert window ticker data"
            );
            return Err(e.into());
        }
        Ok(())
    ...
    async fn write_aggregate_trade(&self, trade: &AggregateTrade) -> Result<()> {
        ...
        .await
        {
            error!(
                symbol = %trade.symbol,
                error = %e,
                "Failed to insert aggregate trade"
            );
            return Err(e.into());
        }
        Ok(())
    ...
    async fn write_orderbook_state(&self, state: &StateSnapshot, symbol: &str) -> Result<()> {
        ...
        .await
        {
            error!(
                symbol = %symbol,
                error = %e,
                "Failed to insert orderbook state"
            );
            return Err(e.into());
        }
        Ok(())
    ...
    async fn write_orderbook_summary(
        ...
        .await
        {
            error!(
                symbol = %symbol,
                error = %e,
                "Failed to insert orderbook summary"
            );
            return Err(e.into());
        }
        Ok(())
    ...
    async fn write_trade_summary(&self, summary: &TradeSummary, symbol: &str) -> Result<()> {
        ...
        .await
        {
            error!(
                symbol = %symbol,
                error = %e,
                "Failed to insert trade summary"
            );
            return Err(e.into());
        }
        Ok(())
...
static CACHED_RESULT: OnceCell<TimescaleWriter> = OnceCell::const_new();
/// Read database settings from infrastructure environment variables
#[must_use]
pub async fn get_writer() -> TimescaleWriter {
    CACHED_RESULT
        .get_or_init(|| async {
            ...
            info!("TimescaleDB settings: {:?}", settings);

            TimescaleWriter::from_config(&settings)
                .await
                .expect("Failed to create TimescaleDB writer")
        })
        .await
        .clone()
}
