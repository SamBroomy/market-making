...
async fn get_binance_client(config: &Config) -> &'static BinanceClient {
    BINANCE_CLIENT
        .get_or_init(|| async { BinanceClient::new(config).await })
        .await
}
...
async fn run_snapshot_task(
    ...
    info!(
        "Starting snapshot task with rate limiting ({}s delay)",
        config.snapshot_delay_seconds
    );
        ...
        tokio::select! {
            ...
            request = snapshot_rx.recv() => {
                if let Some(SnapshotRequest {
                    ...
                    if config.snapshot_delay_seconds > 0 {
                        tokio::select! {
                            () = tokio::time::sleep(config.get_snapshot_delay()) => {},
                            _ = shutdown_rx.recv() => {
                                info!("Snapshot task interrupted during rate limiting delay");
                                let _ = response_tx.send(Err(anyhow::anyhow!("Shutdown requested")));
                                break;
                            }
                        }
                    }
...
pub async fn run_symbol_producer(
    ...
    info!(
        ...
        symbol, config.snapshot_limit
    );

    let bc = get_binance_client(config).await;
    ...
    let signals_producer = if config.enable_signals && config.enable_streaming {
        Some(MessageProducer::new(iggy_client, "orderbook_signals", &symbol, 1).await?)
    } else {
        None
    };

    let state_producer = if config.enable_state && config.enable_streaming {
        Some(MessageProducer::new(iggy_client, "orderbook_state", &symbol, 1).await?)
    } else {
        None
    };

    // Start snapshot task with shutdown handling
    ...
    for (index, symbol) in symbols.into_iter().enumerate() {
        ...
        if index > 0 {
            ...
            info!(
                ...
                config.startup_delay_seconds, symbol
            );
