...
use message_stream::StreamProducer;
use persistence::DataWriter;
use serde::Serialize;
...
pub struct DefaultDataHandler {
    message_producer: Option<StreamProducer>,
    ...
    T: Serialize + Debug + Send,
{
    ...
    pub async fn run(&self, mut receiver: UnboundedReceiver<T>) -> Result<()> {
        info!(
...
use market_core::data_models::{AggregateTrade, DepthUpdate, TickerData, WindowTickerData};
...
pub struct DepthUpdateHandler {
    ...
    orderbook_sender: Option<UnboundedSender<DepthUpdate>>,
}
...
pub struct AggTradeHandler {
    default_handler: DefaultDataHandler,
    trade_processor_sender: Option<UnboundedSender<AggregateTrade>>,
}
